...
export default function Dashboard() {
      ...
      try {
        ...
        try {
          ...
          if (tRes.data && tRes.data.success) {
            ...
            if (me.role === 'penghuni') {
              tPayload = tPayload.filter(t => String(t.Id_Penghuni) === String(me.id));
            }
            setTagihan(tPayload);
            ...
            const totals = {};
            tPayload
              .filter(item => item && String(item.Status).toLowerCase() === 'lunas')
              .forEach(item => {
          ...
          if (rRes.data && rRes.data.success) {
            ...
            if (me.role === 'penghuni') {
              rPayload = rPayload.filter(r => String(r.Id_Penghuni) === String(me.id));
            }
            setRiwayat(rPayload);
          }

          if (sRes.data && sRes.data.success) setStatistics(sRes.data.payload || null);
        } catch (innerErr) {
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="mt-4 px-32 relative">
          <div className="flex gap-16 justify-center">
            <div className="min-w-[402px] min-h-[206px] shadow-md shadow-gray-200 bg-[#0F7AC7] px-4 rounded-4xl flex justify-center items-center">
              <span className="flex-1/2 justify-center flex">
                <IoWalletOutline className="size-24 text-white" />
              </span>
          ...
          <div className="mt-4 bg-[#03C3FD]/27 p-4">
            <h1 className="text-[#0172C4]">
              Info MAse!! Bagi mahasiswa yang sudah melakukan pembayaran harap
      ...
      {/* TAGIHAN */}
      <Tagihan tagihan={tagihan} riwayat={riwayat} statistics={statistics} admin={admin} paidByUser={paidByUser} />
    </div>
  );
}
